...
export default class Ysera {
     ...
     */
    static sendError(req, res, { name, code, message, internal, stack, originalError }) {
        if (code) {
            Ysera.send(res, code, DEV ? stack : { code: internal, message });
        } else {
    ...
    static printStack(stack) {
        const stacks = stack.split('\n'),
            filtered = stacks.filter((line) => {
